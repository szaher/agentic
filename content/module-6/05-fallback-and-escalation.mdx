...
- Every tier result is recorded in `tiers_tried`, creating an **audit trail**. When you get to Module 12's monitoring and cost lessons, this trail becomes invaluable for understanding where your budget goes.
...
**Principle 2: Make thresholds tunable, not hardcoded.** Your initial thresholds will be wrong. In production, you will tune them based on actual resolution rates. Store thresholds in configuration, not in code. Track the distribution of which tier resolves each request and adjust. Module 12 Lesson 06 covers the monitoring infrastructure you need for this.

**Principle 3: Add circuit breakers at each tier.** If Haiku starts returning errors (API outage, rate limit), skip it and go directly to Sonnet. Do not waste time and tokens waiting for a tier that is down. This connects directly to the reliability patterns you will see in Module 12 Lesson 04.
...
And looking ahead to Module 12: the cascade's `tiers_tried` audit trail feeds directly into the cost monitoring and optimization infrastructure you will build there. Every escalation is a data point that helps you tune thresholds, identify patterns in what gets escalated, and measure the true cost per resolution.
...
- Design cascades with **tunable thresholds**, **circuit breakers**, and **audit trails** to enable the production monitoring and cost optimization covered in Module 12
