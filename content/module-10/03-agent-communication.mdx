...
quiz:
  ...
  - question: "What is the primary risk of using shared mutable state (like a blackboard) without access controls?"
    ...
    correctIndex: 2
  - question: "How does the middleware pattern from Module 6 Lesson 3 relate to message-based agent communication?"
    options:
...
This is where the middleware concept from Module 6, Lesson 3 reappears at a new level. In that lesson, middleware intercepted tool calls *within* a single agent. Here, message middleware intercepts messages *between* agents. The same interception principle -- inspect, transform, or block a message as it passes through -- applies at the inter-agent boundary. You can add logging middleware to the bus that records every message, a validation middleware that rejects malformed events, or a rate-limiting middleware that throttles a noisy agent.
...
class MessageMiddleware:
    ...
    as tool-call middleware from Module 6, Lesson 3 -- but applied to
...
- The middleware interception pattern from Module 6, Lesson 3 applies at the inter-agent level too -- a message bus supports the same logging, validation, and rate-limiting middleware, just applied to messages between agents instead of tool calls within a single agent.
