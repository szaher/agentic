...
This is the pattern that the OpenAI Agents SDK uses natively (Module 8 Lesson 02). Each agent declares which other agents it can hand off to, and the runtime manages the transitions. The result feels conversational and adaptive -- the system routes itself without a centralized dispatcher.
...
If you used CrewAI's **sequential process** in Module 8 Lesson 05, you have already built a pipeline. Each CrewAI `Task` is assigned to an `Agent`, and when the process type is `sequential`, tasks execute in order with each task's output feeding into the next. The pipeline topology formalizes this pattern: the structure is the sequence itself.
...
CrewAI's **hierarchical process** (Module 8 Lesson 05) is a two-level hierarchy: a manager agent automatically created by the framework coordinates the crew's agents. The topology described here generalizes that to arbitrary depth -- managers managing managers -- for systems complex enough to warrant it.
...
- **Pipeline** -- agents are arranged in a fixed sequence where each stage transforms and passes data forward. Best for workflows with clear, ordered stages. This maps to CrewAI's sequential process (Module 8 Lesson 05).
